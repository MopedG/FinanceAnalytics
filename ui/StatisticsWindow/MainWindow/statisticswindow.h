...
private:
    ...
    void updateSpendingForms(const std::vector<std::shared_ptr<EntryData> > &data);
    void createMonthCards(const QStringList &dates, bool update = false);
    void createSpendingForms(const std::vector<std::shared_ptr<EntryData>> &data);
